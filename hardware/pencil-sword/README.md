...
| `stl/05_fit_test_rings.stl` | 試し輪 | 3 個 | 3.9 cm³ | 本番前のはめあい確認（後述） |
| `stl/06_bore_gauge.stl` | 測りゲージ | 7 個 | 7.5 cm³ | 鉛筆の対辺がわからないとき（後述） |
...
新品の鉛筆（175mm）がちょうど全部隠れる位置で区切ってある。
...
対象は **JIS 六角鉛筆（対辺 7.8mm・長さ 175mm）**。三菱・トンボの普通の鉛筆はこれ。

長さは **105〜184mm** の範囲で使える。鉛筆のお尻は柄頭の袋穴の底（剣の座標で z=-6.2）に
突き当たり、先端は剣身の袋穴の底（z=178）まで入る余地がある。175mm の鉛筆なら
先端は袋穴の底の 9mm 手前で止まる。

穴は 3 段構えになっている。

- **本体の穴 対辺 8.45mm**（＝鉛筆 + 0.65）… するする通る
- **抜け止めリブ 6 本**（高さ 0.25mm・実効対辺 7.95mm ＝ 鉛筆 + 0.15）… 六角の「面」の真ん中を押さえる
...
穴もリブも鉛筆の対辺からの足し算で決めているので、一発で全部品がついてくる。

```bash
python3 generate.py --pencil-af 7.0
```

外形はまったく変わらず、穴が縮んで肉が厚くなるだけ（7.0mm なら最小肉厚が
1.51 → 1.9mm 台に増える）。細い鉛筆ほど丈夫になる。

### 対辺がわからないとき → `06_bore_gauge.stl`

素の穴（リブなし）の輪が 7 個、一列に並んでいる。**高さで見分ける。**

| 高さ | 6mm | 8mm | 10mm | 12mm | 14mm | 16mm | 18mm |
|---|---|---|---|---|---|---|---|
| 穴 対辺 | 7.10 | 7.35 | 7.60 | 7.85 | 8.10 | 8.35 | 8.60 |

**するっと入る、いちばん小さい穴**を探す。その穴の対辺から 0.3 ほど引いた値が
鉛筆の実際の対辺だと思ってよい（引く分は、刷ったときに穴が小さめに出るぶん）。
そのまま `--pencil-af` に渡す。ノギスがなくてもこれで決まる。

### 最後の詰め → `05_fit_test_rings.stl`

対辺が分かったあと、本番を刷る前の確認用。高さ違いの輪が 3 個。**低いほどきつい。**
...
「押し込んだあと落ちないか」までそのまま確かめられる。15 分ほどで刷れる。

- **11mm がちょうど** → そのまま本番を刷る
- **8mm がちょうど**（＝標準はゆるい） → `SLIDE_GAP` / `RIB_GAP` を 0.25 ずつ下げる
- **14mm がちょうど**（＝標準はきつい） → 同じく 0.25 ずつ上げる
...
python3 generate.py --pencil-af 7.0  # 細い鉛筆に合わせて穴とリブを作り直す
```
